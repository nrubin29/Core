...
public class StatusBar extends JPanel {
	...
	public StatusBar() {
		for (int i = 0; i < Constants.MAX_HEALTH; i++) {
			JLabel heart = new JLabel(Image.HEART.getImage(Constants.STATUSBAR_OBJECT_DIM, Constants.STATUSBAR_OBJECT_DIM));
			add(heart);
		}
		...
		for (int i = 0; i < Constants.MAX_FOOD; i++) {
			JLabel food = new JLabel(Image.FOOD.getImage(Constants.STATUSBAR_OBJECT_DIM, Constants.STATUSBAR_OBJECT_DIM));
			add(food);
		}
